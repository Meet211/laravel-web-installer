    ...
    $.easyAjax = function (options) {
        ...
        if (typeof opt.beforeSend != "function") {
            opt.beforeSend = function () {
                ...
                // Clear Tailwind form-messages container if present
                var fm = $(opt.container).find('#form-messages');
                if (fm.length) {
                    fm.html('');
                }
        ...
        function showResponseMessage(msg, type, toastrOptions) {
            ...
            if (opt.messagePosition == "toastr") {
                ...
                // Prefer Tailwind message container when available
                var formMessages = $(opt.container).find('#form-messages');
                if (formMessages.length) {
                    // Build solid color alert blocks for success (teal) and error (red)
                    var html;
                    if (type === 'success') {
                        var sid = 'hs-solid-color-success-label-' + Date.now();
                        html = '' +
                            '<div class="mt-2 bg-teal-500 text-sm text-white rounded-lg p-4" role="alert" tabindex="-1" aria-labelledby="' + sid + '">' +
                            '  <span id="' + sid + '" class="font-bold">Success</span> ' + msg +
                            '</div>';
                    } else if (type === 'error') {
                        var did = 'hs-solid-color-danger-label-' + Date.now();
                        html = '' +
                            '<div class="mt-2 bg-red-500 text-sm text-white rounded-lg p-4" role="alert" tabindex="-1" aria-labelledby="' + did + '">' +
                            '  <span id="' + did + '" class="font-bold">Danger</span> ' + msg +
                            '</div>';
                    } else {
                        // Fallback subtle alert for other types
                        var twType = {
                            warning: 'yellow',
                            info: 'blue',
                            primary: 'indigo'
                        ...
                        html = '' +
                            '<div class="mb-4 rounded-md border border-' + twType + '-200 bg-' + twType + '-50 px-4 py-3 text-sm text-' + twType + '-800">' +
                            msg +
                            '</div>';
                    }
                    formMessages.html(html);
                } else {
                    // Fallback: still render Tailwind-styled alerts even without #form-messages
                    var ele = $(opt.container).find("#alert");

                    var html;
                    if (type === 'success') {
                        var sid2 = 'hs-solid-color-success-label-' + Date.now();
                        html = '' +
                            '<div class="mt-2 bg-teal-500 text-sm text-white rounded-lg p-4" role="alert" tabindex="-1" aria-labelledby="' + sid2 + '">' +
                            '  <span id="' + sid2 + '" class="font-bold">Success</span> ' + msg +
                            '</div>';
                    } else if (type === 'error') {
                        var did2 = 'hs-solid-color-danger-label-' + Date.now();
                        html = '' +
                            '<div class="mt-2 bg-red-500 text-sm text-white rounded-lg p-4" role="alert" tabindex="-1" aria-labelledby="' + did2 + '">' +
                            '  <span id="' + did2 + '" class="font-bold">Danger</span> ' + msg +
                            '</div>';
                    } else {
                        var twType2 = {
                            warning: 'yellow',
                        ...
                        html = '' +
                            '<div class="mb-4 rounded-md border border-' + twType2 + '-200 bg-' + twType2 + '-50 px-4 py-3 text-sm text-' + twType2 + '-800">' +
                            msg +
                            '</div>';
                    }
        ...
        function handleFail(response) {
            ...
            if (typeof response.errors != "undefined") {
                ...
                if (opt.errorPosition == "field") {
                    ...
                    // Render a grouped error list. Prefer Tailwind top container when available.
                    var errorMsg = "<ul>";
                    for (var i = 0; i < keys.length; i++) {
                    ...
                    if (formMessages.length) {
                        ...
                        var html = '' +
                            '<div class="mt-2 bg-red-500 text-sm text-white rounded-lg p-4" role="alert" tabindex="-1" aria-labelledby="' + did + '">' +
                            '  <span id="' + did + '" class="font-bold">Danger</span> ' + errorMsg +
                            '</div>';
                        formMessages.html(html);
                    } else {
                        // Fallback to Tailwind solid red alert even without #form-messages
                        var errorElement = $(opt.container).find("#alert");
                        var did3 = 'hs-solid-color-danger-label-' + Date.now();
                        var html = '' +
                            '<div class="mt-2 bg-red-500 text-sm text-white rounded-lg p-4" role="alert" tabindex="-1" aria-labelledby="' + did3 + '">' +
                            '  <span id="' + did3 + '" class="font-bold">Danger</span> ' + errorMsg +
                            '</div>';
